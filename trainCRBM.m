...
filtH = params.filterHeight;
filtW = params.filterWidth;
p = params.szPool;
H = size(data.x, 1);
W = size(data.x, 2);
...
if exist('oldModel','var') && ~isempty(oldModel),
    model = oldModel;
    if (~isfield(model,'W')), 
        model.W = 0.01 * randn(filtH, filtW, colors, Nfilters);
    else
        if (size(model.W) ~= [filtH filtW colors Nfilters]), error('Incompatible input model.'); end
    end
...
for iter = model.iter+1:param_iter,
    ...
    for batch = 1:total_batches,
        ...
        ddw = convs4(batchdata(filtH:H-filtH+1,filtW:W-filtW+1,:,:), poshidprobs(filtH:Hhidden-filtH+1,filtW:Whidden-filtW+1,:,:), useCuda) ...
            - convs4(    recon(filtH:H-filtH+1,filtW:W-filtW+1,:,:), neghidprobs(filtH:Hhidden-filtH+1,filtW:Whidden-filtW+1,:,:), useCuda);
        dW = pW * dW + ddw / (Hhidden - 2 * filtH + 2) / (Whidden - 2 * filtW + 2) / param_szBatch;
